...
export default class NavigationStore {
  ...
  Router = types
    .model('Router', {
      ...
      routes: types.maybe(types.array(types.late(() => this.ExtendedRouter))),
      ...
      return {
        ...
        init: ({children, index, initialRouteName, ...props}) => {
          ...
          if (children) {
            ...
            self.routes = children.map(this.createScene)
            self.index = index || 0
            if (initialRouteName) {
  ...
  create = scenes => {
    ...
    return this.createScene(scenes)
  }

  createScene = props => {
    const res = this.ExtendedRouter.create(props)
    res.init(props)
    return res
  }
}
